...
pub trait Program {
    ...
    fn required_modules(&self) -> Result<u8>;
...
mod tests {
    ...
    fn test_header() {
        ...
        assert_eq!(program.required_modules().unwrap(), modules::ModuleFlags::LED.bits());
        assert_eq!(program.program_name().unwrap(), "TestProg");
        assert_eq!(program.program_start().unwrap(), program.len() as u16 - 2);
        assert_eq!(program[program.program_start().unwrap() as usize..], [0xff, 0xff]);
    }
}
